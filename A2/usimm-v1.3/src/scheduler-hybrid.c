...
void schedule(int channel)
{
	...
	if(drain_writes[channel])
	{
		...
		LL_FOREACH(write_queue_head[channel], wr_ptr)
		{
			if(wr_ptr->command_issuable)
			{
				...
				if(current_page_policy[channel] == CLOSE_POLICY)
				{
				...
				// If next command is COL_WRITE_CMD, the check of hit/ miss depending on the current policy, and change counter_hybrid accordingly.
				if(wr_ptr->next_command == COL_WRITE_CMD)
				{
					if(row != previous_row[channel] && current_page_policy[channel] == OPEN_POLICY && counter_hybrid[channel] < 16)
					{
						counter_hybrid[channel] += 1; 
					}
				
					if(row == previous_row[channel] && current_page_policy[channel] == CLOSE_POLICY && counter_hybrid[channel] > 0)
					{
						counter_hybrid[channel] -= 1; 
					}
					previous_row[channel] = row;
				}
	...
	if(!drain_writes[channel])
	{
		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			if(rd_ptr->command_issuable)
			{
				...
				// If next command is COL_READ_CMD, the check of hit/ miss depending on the current policy, and change counter_hybrid accordingly.
				if(rd_ptr->next_command == COL_READ_CMD)
				{
					if(row != previous_row[channel] && current_page_policy[channel] == OPEN_POLICY && counter_hybrid[channel] < 16)
					{
	...
	// To make a proper close-page policy, the command_issued_current_cycle[channel] command has been removed. 
	// This issues PRE command whenever COL_READ_CMD or COL_WRITE_CMD is issued.
	if(current_page_policy[channel] == CLOSE_POLICY)
	{
	...
	if(current_page_policy[channel] == OPEN_POLICY && counter_hybrid[channel] >= THRESHOLD_HIGH_HYBRID)
	{
		current_page_policy[channel] = CLOSE_POLICY;
	}

	if(current_page_policy[channel] == CLOSE_POLICY && counter_hybrid[channel] <= THRESHOLD_LOW_HYBRID)
	{
		current_page_policy[channel] = OPEN_POLICY;
	}
}
